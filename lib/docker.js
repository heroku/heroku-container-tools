...
function runImage(imageId, cwd, command, mount) {
  ...
  var commandWithPort = command.replace("$PORT", 3000).replace("${PORT}, 3000")
  var runCommand = `run -w /app/src -p 3000:3000 --rm -it ${mountComponent} ${envArgComponent} ${imageId} sh -c "${commandWithPort}" -w /app || true`;
