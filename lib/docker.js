...
const USER_DIR = '/app/user';
const FILENAME = 'Dockerfile'; // TODO: change to heroku-Dockerfile once 1.3.0 is released
const COMPOSE_FILENAME = 'docker-compose.yml'; // TODO: consider renaming to heroku-compose.yml
const PORT = 8080;

module.exports = {
  filename: FILENAME,
  composeFilename: COMPOSE_FILENAME,
  userDir: USER_DIR,
  port: PORT,
  silent: false,
...
function runImage(imageId, cwd, command, mount) {
  ...
  var mountComponent = mount ? `-v "${mountDir}:${USER_DIR}"` : '';
  var envArgComponent = directory.getFormattedEnvArgComponent(cwd);
  var commandWithEnv = command.replace("$", "\\\$")
  var runCommand = `run -p ${ PORT }:${ PORT } -e "PORT=${ PORT }" --rm -it ${mountComponent} ${envArgComponent} ${imageId} sh -c "${commandWithEnv}" || true`;
